...
my $ignorelist_file = "ignorelist.txt";

my $debug = 1;

my ( @ignorelist, @idiots );
...
sub get_rate_limit {
    ...
    if ( $m->{'resources'}->{'application'}->{'/application/rate_limit_status'}
        ->{'remaining'} == 0 )
    {
        print " -- API limit reached, waiting for "
          . ( $m->{'resources'}->{'application'}
              ->{'/application/rate_limit_status'}->{'reset'} - time )
          . " seconds --\n"
          if $debug;
        sleep( $m->{'resources'}->{'application'}
              ->{'/application/rate_limit_status'}->{'reset'} - time + 1 );
    }

    if ( $type =~ /followers/ ) {
        return {
            remaining => $m->{'resources'}->{'followers'}->{'/followers/ids'}
              ->{'remaining'},
            reset =>
              $m->{'resources'}->{'followers'}->{'/followers/ids'}->{'reset'}
        };
    }
    elsif ( $type =~ /lookup_users/ ) {
        return {
            remaining =>
              $m->{'resources'}->{'users'}->{'/users/lookup'}->{'remaining'},
...
sub is_ignorelisted {
    ...
    if (@ignorelist) {
        foreach my $notasheep (@ignorelist) {
...
foreach my $sheep ( keys %problem ) {
    next if is_ignorelisted( $problem{$sheep}->{'name'} );
